...
typedef struct MyAVPacketList {
    ...
    int			serial;     //播放序列      主要应⽤于seek操作
} MyAVPacketList;

typedef struct PacketQueue {//感觉只维护两个链表
...
//音视频都有自己的clock，以哪个为基准哪个就是主时钟，主时钟可以set，其余只能get主时钟进行同步。
...
typedef struct Clock {
    ...
    //ffplay使用double标识pts，但是内部不会使用double。
    double	pts_drift;      // clock base minus time at which we updated the clock
    //相对于系统时钟的差值。不同的clock有不同的pts_drift，
...
typedef struct Frame {
    ...
    int64_t		pos;            // 该帧在输入文件中的字节位置。seek既可以使用字节定位也可以使用时间。
    int		width;              // 图像宽度
    int		height;             // 图像高读
...
typedef struct FrameQueue { //!使用数组的方式组织环形队列，只能入队不能出队。
    //这种结构允许高效管理帧数据，而无需频繁清空和重新分配内存。
    //!使用数据来实现队列，适合于明确最大容量 max_size。像pkt的队列很难确定pkt数量。
    Frame	queue[FRAME_QUEUE_SIZE];        // FRAME_QUEUE_SIZE  最大size, 数字太大时会占用大量的内存，需要注意该值的设置。 //!这个FRAME_QUEUE_SIZE 大小只是定义的队列大小，实际设置大小是max_size决定的。
    int		rindex;                         // 读索引。待播放时读取此帧进行播放，播放后此帧成为上一帧      
    //!rindex 感觉应该定义为上一帧 rindex+rindex_shown 才是当前帧。这里的 上一帧 其实是当前播放的帧
    int		windex;                         // 写索引
    int		size;                           // 当前总帧数
    int		max_size;                       // 可存储最大帧数
    int		keep_last;                      // = 1说明要在队列里面保持最后一帧的数据不释放，只在销毁队列的时候才将其真正释放。这个还不如定义为bool.
    //这个定义为保留上一帧，肯定为1 要不然peek_last函数用不了。
    int		rindex_shown;                   // 初始化为0，配合keep_last=1使用
    SDL_mutex	*mutex;                     // 互斥量
    SDL_cond	*cond;                      // 条件变量
    PacketQueue	*pktq;                      // 数据包缓冲队列       为什么这又搞个pkt的queue
} FrameQueue;
...
typedef struct VideoState {
    ...
    int		queue_attachments_req;  //有的音频带有图片 类似专辑封面
    int		seek_req;           // 标识一次seek请求 每次拖动或点击 都会置1
    int		seek_flags;         // seek标志，诸如AVSEEK_FLAG_BYTE等
    int64_t		seek_pos;       // 请求seek的目标位置(绝对位置)
    int64_t		seek_rel;       // 本次seek的位置增量(相对位置)
    int		read_pause_return;
    AVFormatContext *ic;        // iformat的上下文
    int		realtime;           // =1为实时流

    Clock	audclk;             // 音频时钟
    Clock	vidclk;             // 视频时钟 一般只有媒体中只有视频才会以视频为基准
    Clock	extclk;             // 外部时钟
    ...
    //audio_hw_buf_size 这个对音视频同步精度是有影响的
    // 指向待播放的一帧音频数据，指向的数据区将被拷入SDL音频缓冲区。若经过重采样则指向audio_buf1，
    // 否则指向frame中的音频
    uint8_t			*audio_buf;             // 指向需要重采样的数据
    uint8_t			*audio_buf1;            // 指向重采样后的数据
    unsigned int		audio_buf_size;     // 重采样后pcm音频数据的大小
    unsigned int		audio_buf1_size;    // 申请到的音频缓冲区audio_buf1的实际尺寸。最大缓冲长度
    int			audio_buf_index;            // 当前缓冲区的位置
    // 的位置索引(指向第一个待拷贝字节)
    // audio_buf_size = audio_buf_index + audio_write_buf_size
    int			audio_write_buf_size;// 当前音频帧中尚未拷入SDL音频缓冲区的数据量:
    int			audio_volume;               // 音量
    int			muted;                      // =1静音，=0则正常
    struct AudioParams audio_src;           // 音频解码后的frame的参数
#if CONFIG_AVFILTER
    struct AudioParams audio_filter_src;
#endif
    struct AudioParams audio_tgt;       // 目标SDL支持的音频参数，重采样转换：audio_src->audio_tgt
...
static int packet_queue_put_private(PacketQueue *q, AVPacket *pkt)
{
    ...
    if (q->abort_request)   //如果已中止，则放入失败
        return -1;
                                                //get的时候free
    pkt1 = av_malloc(sizeof(MyAVPacketList));   //分配节点内存。我认为应该这个队列自己维护了pkt1的内存释放。
    if (!pkt1)  //内存不足，则放入失败
        return -1;
    ...
    if (!q->last_pkt)   //如果last_pkt为空，说明队列是空的，新增节点为队头
        q->first_pkt = pkt1;
    else                //不是新的，就把新创建的pkt1塞到next。
        q->last_pkt->next = pkt1;   //把新的pkt1塞到链表中
    q->last_pkt = pkt1; //更新队列尾操作，即使是新列表，让first和last同时指向pkt1.
    //第一轮start之后last肯定不是空的。

    //队列属性操作：增加节点数、cache大小、cache总时长, 用来控制队列的大小
    q->nb_packets++;
    q->size += pkt1->pkt.size + sizeof(*pkt1);  //这个大小是总大小
...
static int packet_queue_put(PacketQueue *q, AVPacket *pkt)
{
    ...
    if (pkt != &flush_pkt && ret < 0)   //flush_pkt不参与释放
...
static int packet_queue_init(PacketQueue *q)
{
    ...
    q->abort_request = 1;   //终止播放先设为1，后面在start会恢复。
    return 0;
}
...
static int packet_queue_get(PacketQueue *q, AVPacket *pkt, int block, int *serial)
{
    ...
    for (;;) {
        ...
        if (pkt1) {             //队列中有数据
            ...
            *pkt = pkt1->pkt;   //给输出的pkt赋值。 取出数据和加入数据一样都是浅拷贝，不增加引用计数。
            if (serial) //如果需要输出serial，把serial输出
                *serial = pkt1->serial; //这里将自定义的pkt和pktqueue序列进行统一
            ...
            SDL_CondWait(q->cond, q->mutex);//死等
        }
    }
...
//专门封装的函数 接收 得到 发送 三部分很有关联
static int decoder_decode_frame(Decoder *d, AVFrame *frame, AVSubtitle *sub) {
    int ret = AVERROR(EAGAIN);

    for (;;) {//这个死循环保证一定能拿到frame，pkt队列没有就cond唤醒
        AVPacket pkt;
        // 1. 流连续情况下获取解码后的帧  刚开始不相同 1 ！= -1 ，所以
        if (d->queue->serial == d->pkt_serial) { //! 1.1 先判断是否是同一播放序列的数据
            //如果不是就去下面从queue获取一个pkt然后sendpkt
            do {
                if (d->queue->abort_request)
                    return -1;  // 是否请求退出
                // 1.2. 获取解码帧
                switch (d->avctx->codec_type) {
                case AVMEDIA_TYPE_VIDEO:   //!先receive后getpkt最后再send，这样反直觉的设计是为了解码器缓冲区。
                    ret = avcodec_receive_frame(d->avctx, frame);
//先进行了解码操作，但是在此之前没有av_read_frame，所以第一次肯定会错误，条件变量让别的线程送数据。
                    if (ret >= 0) { //!第一次flushpkt receive -11肯定失败，直接break 退出switch
                    //decoder_reorder_pts 自动选择合适时间戳，一般都是-1.
                        if (decoder_reorder_pts == -1) {
                    //best_effort_timestamp最接近真实 PTS 的时间戳
                            frame->pts = frame->best_effort_timestamp;
                        } else if (!decoder_reorder_pts) {
                            frame->pts = frame->pkt_dts;//
    //解码时间直接赋值给显示时间 忽略了 I 帧和 B 帧的时间戳差异，可能导致显示顺序错误。
                        }
                    }
                    break;
                ...
                case AVMEDIA_TYPE_AUDIO:
                    ret = avcodec_receive_frame(d->avctx, frame);
                    //第一个flushpkt 返回的是 ret = -11
                    if (ret >= 0) {
                        AVRational tb = (AVRational){1, frame->sample_rate};  
                        if (frame->pts != AV_NOPTS_VALUE) {
// 如果frame->pts正常则先将其从pkt_timebase转成{1, frame->sample_rate}
// pkt_timebase实质就是stream->time_base
// 奇怪，frame的pts一直都是0，应该是可以进来这个判断的。
                            frame->pts = av_rescale_q(frame->pts, d->avctx->pkt_timebase, tb);
//!更新！pkt_timebase这里的无条件转为自己指定的 tb。很奇怪 为什么是只针对音频的？因为视频交由ffmpeg底层算法确定
//!这里转换后pts还只是时间戳，在正式插入队列之前还需要转为 秒单位
                            //!把frame的显示时间戳转为 解码器时间戳
        //!音视频frame的时间戳处理不一样
        //! 音频时间戳处理需要考虑后面的同步，所以需要将编码器的时间基转为采样率的时间基。
                ...
                if (ret == AVERROR_EOF) {
                    d->finished = d->pkt_serial;//记录最终的序列号
        ...
        do {//!播放序列不一样 cond唤醒新送进来pkt
            // 2.1 如果没有数据可读则唤醒read_thread, 实际是 continue_read_thread SDL_cond
            if (d->queue->nb_packets == 0)          // 没有数据可读
                SDL_CondSignal(d->empty_queue_cond);// 通知read_thread放入packet
            // 2.2 如果还有pending的packet则使用它
            if (d->packet_pending) { //这个咋感觉一直是0
                av_packet_move_ref(&pkt, &d->pkt);
                d->packet_pending = 0;  //解码器异常状态。
            } else {
                // 2.3 阻塞式读取packet
            ...
            if(d->queue->serial != d->pkt_serial) {
                ...
                av_packet_unref(&pkt); //!疑似bug  释放要过滤的packet
            }
        } while (d->queue->serial != d->pkt_serial);// 如果不是同一播放序列(流不连续)则继续读取
        //do while。先取出来一个，如果不是一个序列的 就终止。

        //!下面是send pkt。     //debug 取出来了一个pkt但是是flush，可能得等第二个才行。
        // 3 将packet送入解码器
        if (pkt.data == flush_pkt.data) {
            avcodec_flush_buffers(d->avctx); //清空解码器里的缓存帧
            d->finished = 0;        // 重置为0
            d->next_pts = d->start_pts;     // 主要用在了audio
            d->next_pts_tb = d->start_pts_tb;// 主要用在了audio
        } else {
            if (d->avctx->codec_type == AVMEDIA_TYPE_SUBTITLE) {//字幕特殊处理
                int got_frame = 0;
...
static int frame_queue_init(FrameQueue *f, PacketQueue *pktq, int max_size, int keep_last)
{
    ...
    //!两个！！让keep_last 要么为0 要么为1 不能是其余数字。
...
static void frame_queue_destory(FrameQueue *f)
{
    ...
    for (i = 0; i < f->max_size; i++) {
        ...
        frame_queue_unref_item(vp); //这里怎么是引用计数？难道不和pkt一样进行浅拷贝？
        // 释放vp->frame对象
...
//!这种带peek的只是拿出来看看，并不更新读索引。只有frame_queue_next是更新读索引。
{               //数组组织的环形队列的读取操作
 ...
 */
static Frame *frame_queue_peek_last(FrameQueue *f)  //获取上⼀Frame
{
...
{
    SDL_LockMutex(f->mutex);
    while (f->size >= f->max_size && //这里没有 f->size-f->rindex_shown 因为是要计算真正的数据
...
// 更新写指针
static void frame_queue_push(FrameQueue *f) //代码不全--->代码是全的，在queue_picture函数里进行了入队操作。 
{
    if (++f->windex == f->max_size)
        f->windex = 0;  //环形队列 写索引更新
    SDL_LockMutex(f->mutex);
    f->size++;
...
static void frame_queue_next(FrameQueue *f)//!更新读索引rindex frame_queue_push更新写索引
{
    if (f->keep_last && !f->rindex_shown) {
        f->rindex_shown = 1; // 第一次进来没有更新，对应的frame就没有释放
        return; //第一次进来直接return
    }
    frame_queue_unref_item(&f->queue[f->rindex]);//把上一帧给释放了
    if (++f->rindex == f->max_size)//环形队列到头了
        f->rindex = 0;      //环形队列读索引更新
    SDL_LockMutex(f->mutex);
    f->size--;
    SDL_CondSignal(f->cond);
    SDL_UnlockMutex(f->mutex);
...
{ //因为一直保留着last上一帧，但是上一帧已经播放了，所以要减去才是为显示的帧数量。
 ...
 * @param scr_width 窗口宽度        屏幕宽度
 * @param scr_height窗口高度        屏幕高度
 * @param pic_width 显示帧宽度
 * @param pic_height显示帧高度
 * @param pic_sar   显示帧宽高比    像素格式一般是1：1
 */
                                   ...
                                   int pic_width, int pic_height, AVRational pic_sar)
{
    ...
    if (av_cmp_q(aspect_ratio, av_make_q(0, 1)) <= 0)//对比两个宽高比大小
        aspect_ratio = av_make_q(1, 1);
        // 如果aspect_ratio是负数或者为0,证明输入的宽高比非法，设置为1:1
    // 转成真正的播放比例
    ...
    if (width > scr_width) {//如果计算的width超过了屏幕大小，就以宽度为基准。
    ...
    rect->x = scr_xleft + x;    //1073741183 感觉这个是一个异常值
    rect->y = scr_ytop  + y;    //0
...
//    rect.w = rect.w /2;   //! 缩放实际不是用sws， 缩放是sdl去做的
    if (!vp->uploaded) {//没有显示过 就上传到texture
...
{
    SDL_Rect rect;      //没有指定任何参数，所以两个都是 INT_MAX
    ...
    //不指定宽度 可能是宽高比sar 所以指定一个高度就可以了 而且如果设置宽度可能会改变宽高比
...
// ! set at get 三个一块看  假如怎么才能
//! 最主要的是diff 
/*!
假如以音频为基准，由于音频每隔一个采样间隔才更新时钟，所以不能让视频直接以音频的为主，必须引入一个系统时间为主，但由于是以音频为基准，所以将音频和系统时钟的差值diff作为一个参考，才能让视频让视频帧准时播放。
例如：刚开始音频pts0，系统-1000，diff=-1000，23ms后diff认为仍然差不多-1000左右，视频frame pts=40ms在一直判断get_time（得到现在的系统时间减去音频clock的diff）函数返回是否在40ms附近。

return c->pts_drift + time 直接return 也可以，后面那个考虑了播放速度
*/
...
{               //这里获取系统时间是微妙
    double time = av_gettime_relative() / 1000000.0;
    //av_gettime_relative() 返回的是自某个时间的微妙，其实绝对时间不重要，只是为了算相对时间
    //除以1000000转换为秒
...
static void init_clock(Clock *c, int *queue_serial)
{
    c->speed = 1.0;
    c->paused = 0;
    c->queue_serial = queue_serial;//刚开始queue初始化的序列设置的是0
    set_clock(c, NAN, -1);//刚开始init初始化的序列设置的是-1
}
...
retry:  //goto构成一个循环
        if (frame_queue_nb_remaining(&is->pictq) == 0) {// 帧队列是否为空
            ...
            // 经过 compute_target_delay 方法，计算出待显示帧vp需要等待的时间
            // 如果以video同步，则delay直接等于last_duration。
            // 如果以audio或外部时钟同步，则需要比对主时钟调整待显示帧vp要等待的时间。
            //! 当前帧可能已经播放了一段时间
            delay = compute_target_delay(last_duration, is); // 当前帧还需要播放多久
            time= av_gettime_relative() / 1000000.0;
...
static int get_video_frame(VideoState *is, AVFrame *frame)
{
    ...
    if (got_picture) {
        ...
        if (framedrop > 0 || // 允许drop帧
                (framedrop && get_master_sync_type(is) != AV_SYNC_VIDEO_MASTER))//非视频同步模式，视频为主的话是不会drop视频帧
        {                       
            if (frame->pts != AV_NOPTS_VALUE) { // pts值有效
                double diff = dpts - get_master_clock(is);//当前frame时间-主时钟
                if (!isnan(diff) &&     // 差值有效
                        fabs(diff) < AV_NOSYNC_THRESHOLD && //! 差值在可同步范围，才drop。差的太多就不drop？？
//因为它们距离“正确”的播放时间非常接近，丢弃它们对观看体验的影响相对较小
//当前帧与主时钟的差距已经超出了正常的同步范围，这种情况下通常不会选择丢弃帧，因为这样做可能会导致视频的播放出现明显的跳跃或者不连贯，影响观看体验。
//!在以视频为基准 音频同步到视频的函数中，和这个解释完全不一样
...
static int audio_thread(void *arg)
{
    ...
    do {
        // 1. 读取解码帧
        //decoder_decode_frame 先做了接收解码后的frame 后送入pkt。
        //其实这里顺序不再重要，反正是死循环，取出数据就好
        ...
        if (got_frame) {
            ...
            while ((ret = av_buffersink_get_frame_flags(is->out_audio_filter, frame, 0)) >= 0) {
                ...
                av_frame_move_ref(af->frame, frame);    //实现frame入队
                frame_queue_push(&is->sampq);//更新写索引
//为什么video_thread线程里进行了frame的入队操作 queue_picture，audio_thread 只进行了更新写索引。
...
static int decoder_start(Decoder *d, int (*fn)(void *), const char *thread_name, void* arg)
{
    ...
    d->decoder_tid = SDL_CreateThread(fn, thread_name, arg);    // 启动解码线程
    if (!d->decoder_tid) {                  //thread_name只是SDL里一个名字
...
static int video_thread(void *arg)
{
    ...
    //为啥这里的tb是1：16000，下面的frame_rate是30
    ...
    for (;;) {  // 循环取出视频解码的帧数据
        ...
        //这个函数内部avcodec_receive_frame -> packet_queue_get -> avcodec_send_packet
        //这种反直觉的设计主要考虑 解码器缓冲区不要阻塞。
        if (ret < 0)
            goto the_end;   //解码结束, 什么时候会结束
        if (!ret)  //没有解码得到画面, 什么情况下会得不到解后的帧
            continue;//ret=0 可能帧被drop了，继续读。
#if CONFIG_AVFILTER
...
#endif
            // 4 计算帧持续时间和换算pts值为秒
            // 1/帧率 = duration 单位秒, 没有帧率时则设置为0, 有帧率帧计算出帧间隔0.0333333
            duration = (frame_rate.num && frame_rate.den ? av_q2d((AVRational){frame_rate.den, frame_rate.num}) : 0);
            // 根据AVStream timebase计算出pts值, 单位为秒
            pts = (frame->pts == AV_NOPTS_VALUE) ? NAN : frame->pts * av_q2d(tb);
            // 5 将解码后的视频帧插入队列
            ret = queue_picture(is, frame, pts, duration, frame->pkt_pos, is->viddec.pkt_serial);
            //!对于视频需要额外的处理，所以在这个函数里做了额外的设置。音频只需要更新写索引即可。
            // 6 释放frame对应的数据
...
{
    int data_size, resampled_data_size; //字节大小
    int64_t dec_channel_layout;
    ...
    do {
#if defined(_WIN32)         
        while (frame_queue_nb_remaining(&is->sampq) == 0) {//如果没有帧就一直sleep，直到有帧
...
#endif
        // 若队列头部可读，则由af指向可读帧
        if (!(af = frame_queue_peek_readable(&is->sampq)))//用af指向可读帧
            return -1;
        frame_queue_next(&is->sampq);   //更新读索引
    ...
    if (af->frame->format           != is->audio_src.fmt            || // 采样格式
            ...
            // 第4个条件, 需要改变样本数量, 那就是需要初始化重采样
            (wanted_nb_samples      != af->frame->nb_samples && !is->swr_ctx) 
            // samples不同且swr_ctx没有初始化
            ) {
//!为什么要和src的比？因为在外面函数已经把 is->audio_src = is->audio_tgt
//!第一次判断 src和tag是一样的 下面有对is->audio_src进行修改，第二次进来就是判断解码后参数是不是和以前一样
        swr_free(&is->swr_ctx);
    ...
    if (is->swr_ctx) {
        ...
        // 重采样输出参数2：输出音频缓冲区
            //1280
        int out_count = (int64_t)wanted_nb_samples * is->audio_tgt.freq / af->frame->sample_rate + 256; // + 256 是最大缓存数据，空间留充足
        ...
        //以下这个好像是有专门的api进行补偿的
        if (wanted_nb_samples != af->frame->nb_samples) {   //正常这个是相等的，否则就需要补偿
        ...
        //debug 1024  5120
        if (len2 == out_count) {    //+256还刚刚好，说明buffer太小
...
#endif
    return resampled_data_size; //返回重采样的字节大小
}
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
    ...
    audio_callback_time = av_gettime_relative(); // 害怕while可能产生延迟。

    while (len > 0) {   // 循环读取，直到读取到足够的数据
        /* (1)如果is->audio_buf_index < is->audio_buf_size则说明上次拷贝还剩余一些数据，
         * 先拷贝到stream再调用audio_decode_frame
         * (2)如果audio_buf消耗完了，则调用 audio_decode_frame 重新填充audio_buf
         */
        if (is->audio_buf_index >= is->audio_buf_size) {//buf里数据不够，重新写入
            audio_size = audio_decode_frame(is);//返回重采样的字节大小
            if (audio_size < 0) {
                //这个是静音的逻辑
                is->audio_buf = NULL;
        ...
        if (len1 > len)  // 缓冲区数据大于需要的。
            len1 = len;     //需要3000，但现在有4096，就更新len1
        //根据audio_volume决定如何输出audio_buf
        /* 判断是否为静音，以及当前音量的大小，如果音量为最大则直接拷贝数据 */
        
        //原始数据可能是最大的音量。
        /*如果audio_volume为最⼤⾳量，则只需memcpy复制给stream即可。否则，可以利⽤SDL_MixAudioFormat进⾏⾳量调整和混⾳*/
        //!解码后的数据就是最大音量
...
//可能一个音频帧是要 两个 SDL缓冲区 和 一个 audio_write_buf_size（frame未写入数据）
        set_clock_at(&is->audclk, is->audio_clock -
    //SLD是由两个缓冲区（内部和外部缓冲区），一个正在播放，一个是缓冲，再加上audio_buf
    //当前帧pts+duration-
...
static int audio_open(void *opaque, int64_t wanted_channel_layout,
                      ...
                      //audio_hw_params 是输出sdl音频参数
{//传入的wanted_nb_channels 、wanted_sample_rate、wanted_channel_layout都是解码器默认值
    ...
    if (env) {  //如果环境变量有设置，优先从环境变量取得声道数和声道布局
        wanted_nb_channels = atoi(env);
        wanted_channel_layout = av_get_default_channel_layout(wanted_nb_channels);
    }
        //如果外部传的解码器 声道数和声道布局不匹配，更新声道布局
    if (!wanted_channel_layout || wanted_nb_channels != av_get_channel_layout_nb_channels(wanted_channel_layout)) {
        wanted_channel_layout = av_get_default_channel_layout(wanted_nb_channels);
        wanted_channel_layout &= ~AV_CH_LAYOUT_STEREO_DOWNMIX;
    }
    wanted_nb_channels = av_get_channel_layout_nb_channels(wanted_channel_layout);
    // 根据channel_layout获取nb_channels，当传入参数 wanted_nb_channels 不匹配时，此处会作修正
    ...
    //如果有需要的采样频率，其实找的就是44100。如果没有就降低采样率（降采样）。
    //升采样后的音频质量不会超过原始的低采样率音频。
     ...
     */ 
    //音频缓冲区大小 这个算的是2048
    //SDL_AUDIO_MAX_CALLBACKS_PER_SEC 每秒音频最大回调次数
    //log2 44100/30 = 10.53
    //2<< 10.53 = 2的11次方 2048
    wanted_spec.samples = FFMAX(SDL_AUDIO_MIN_BUFFER_SIZE,
                                2 << av_log2(wanted_spec.freq / SDL_AUDIO_MAX_CALLBACKS_PER_SEC));
    //   wanted_spec.samples 每次回调建议的获取的采样点个数 回调太频繁也影响效率，一般设置为两帧
    wanted_spec.callback = sdl_audio_callback;  //通过回调函数读取数据
    ...
    while (!(audio_dev = SDL_OpenAudioDevice(NULL, 0, &wanted_spec, &spec, SDL_AUDIO_ALLOW_FREQUENCY_CHANGE | SDL_AUDIO_ALLOW_CHANNELS_CHANGE))) {
        //只有打开失败了才会进入以下循环体，debug发现即使失败spec也会被赋值属性
        //!如果打开不成功 以下在检测其余的可不可以 继续打开
    ...
    /* 计算一个采样点占用的字节数 */ // 4个字节  ：s16 2
    //采样点字节数随采样格式和声道数影响 两者相乘。

    audio_hw_params->frame_size = av_samples_get_buffer_size(NULL, audio_hw_params->channels, 1, audio_hw_params->fmt, 1);
    /* 计算一秒占用的字节数 */ //44100*4 = 176400
    ...
    //这里难道缓冲区有2028个采样点，才可以算出 2048*4=8192
...
static int stream_component_open(VideoState *is, int stream_index)
{
    ...
    int ret = 0;
    int stream_lowres = lowres; //这个没有设置是 0
    ...
    if (ret < 0)
        goto fail;
    // 设置 pkt_timebase 1：44100
    avctx->pkt_timebase = ic->streams[stream_index]->time_base;
    //!这个只是针对音频的
    //!avcodec_parameters_to_context 还需要额外设置时间基
    //这里在做播放器那节暂时给注释了,这里不能注释。因为在音频解码线程中会用 pkt_timebase
    //把frame 的pts 无条件转为流的 pkt_timebase
    ...
    case AVMEDIA_TYPE_AUDIO: 
        ...
        forced_codec_name = audio_codec_name; //没有设置为 0
        break;
    case AVMEDIA_TYPE_SUBTITLE:
    ...
    avctx->codec_id = codec->id;//这个感觉不用设置，debug运行前后没有变化
    ...
    switch (avctx->codec_type) {
    case AVMEDIA_TYPE_AUDIO:
#if CONFIG_AVFILTER         //filter 暂时不考虑。
    {
        ...
        channel_layout = avctx->channel_layout; //视频59分
#endif
        ...
        is->audio_stream = stream_index;    // 获取audio的stream索引  1
        is->audio_st = ic->streams[stream_index];  // 获取audio的stream指针
        // 初始化ffplay封装的音频解码器. 对is做了一些绑定操作。
        ...
        if ((ret = decoder_start(&is->auddec, audio_thread, "audio_decoder", is)) < 0)
            goto out;
        SDL_PauseAudioDevice(audio_dev, 0);//这个地方可能会调用音频的回调函数
        break;
    case AVMEDIA_TYPE_VIDEO:
...
static int read_thread(void *arg)
{
    ...
    /* 当执行耗时操作时（一般是在执行while或者for循环的数据读取时），会调用 interrupt_callback.callback
     * 回调函数中返回1则代表ffmpeg结束耗时操作退出当前函数的调用
     * 回调函数中返回0则代表ffmpeg内部继续执行耗时操作，直到完成既定的任务(比如读取到既定的数据包)
     */
    ic->interrupt_callback.callback = decode_interrupt_cb;  //自定义解码超时等待函数
    ic->interrupt_callback.opaque = is;//! opaque 用户可以存储用户自定义的数据或状态，也可以传递数据
                                        //这里就是把is传递过去了。
    //特定选项处理
    ...
    if (find_stream_info) {
        ...
        //avformat_open_input更为详细， 其本质上是去做了decdoe packet获取信息的工作
        err = avformat_find_stream_info(ic, opts);

        for (i = 0; i < orig_nb_streams; i++)
            av_dict_free(&opts[i]);         //里面是按照orig_nb_streams数量malloc dict的。
        av_freep(&opts);
    ...
    if (seek_by_bytes < 0) {//没有配置任何参数进入了这个循环
        int flag = ic->iformat->flags & AVFMT_TS_DISCONT; //时间戳不连续标志位
    ...
    /* 5. 检测是否指定播放起始时间 */
    if (start_time != AV_NOPTS_VALUE) { //没有指定所以没有进来
        int64_t timestamp;

        timestamp = start_time;

        if (ic->start_time != AV_NOPTS_VALUE)
            timestamp += ic->start_time; //!大部分视频起始时间都是0，但也有不是0的。
        // seek的指定的位置开始播放
    ...
    // 6. 查找AVStream
    // 6.1 根据用户指定来查找流。 
    //如果 wanted_stream_spec 没有设置，底下两个循环不起作用，使用av_find_best_stream查询。
    for (i = 0; i < ic->nb_streams; i++) {//找到符合的流
    ...
    for (i = 0; i < AVMEDIA_TYPE_NB; i++) {//有的流不符合
...
//    int wanted_stream_nb, 当一个媒体中有多个音频或视频流，使用者知道需要哪个，所以指定。
//    int related_stream,寻找与当前音频或视频流最合适的视频或音频流。
    if (!audio_disable)
    ...
    if (st_index[AVMEDIA_TYPE_VIDEO] >= 0) {
        ...
        //根据流和帧宽高比猜测视频帧的像素宽高比（像素的宽高比，注意不是图像的）该值只是⼀个参考
        AVRational sar = av_guess_sample_aspect_ratio(ic, st, NULL);
        //像素宽高比一般是1：1
        if (codecpar->width) {
            // 设置显示窗口的大小和宽高比
            set_default_window_size(codecpar->width, codecpar->height, sar);
            //如果设置了播放大小 xy，就会以宽或高为基准来铺满屏幕，但是另一个仍要以满足原始的宽高比进行计算
        }
    }
    ...
    for (;;) {
        ...
        if (is->seek_req) { // 是否有seek请求
            ...
            if (ret < 0) {
                ...
                /* seek的时候，要把原先的数据清空，并重启解码器，put flush_pkt的目的是告知解码线程需要
                 * reset decoder 为什么不清空frame queue
                 */
        ...
        // 4 检测video是否为attached_pic 专辑mp3
        ...
        if (infinite_buffer<1 &&
            ...
            // 如果没有唤醒则超时10ms退出，比如在seek操作时这里会被唤醒.缓存10ms
        ...
        if (!is->paused // 非暂停
                ...
                (!is->audio_st // 没有音频流
                        //下面这个在判断packet队列EOF的时候 会赋值，所以如果结束 肯定是可以的
                 || (is->auddec.finished == is->audioq.serial // 或者音频播放完毕
                     && frame_queue_nb_remaining(&is->sampq) == 0)) //packet和frame队列都为空认为播放完毕
                && (!is->video_st // 没有视频流
...
static VideoState *stream_open(const char *filename, AVInputFormat *iformat)
{
    ...
    if (!is->filename)
        goto fail;
    is->iformat = iformat;//一般都不自定义输入格式
    is->ytop    = 0;
    is->xleft   = 0;
...
static void refresh_loop_wait_event(VideoState *is, SDL_Event *event) {
    ...
    /* 调用SDL_PeepEvents前先调用SDL_PumpEvents，将输入设备的事件加入事件队列中 */
    SDL_PumpEvents();
    /*
...
static void event_loop(VideoState *cur_stream)
{
    ...
    for (;;) {
        ...
        case SDL_KEYDOWN:	/* 键盘事件 */
            ...
            case SDLK_DOWN:
                incr = -60.0;
do_seek:    //seek 一个是通过字节 一个是时间戳
            //时间戳就是直接加减 字节是 加减 时间*码率/8
                if (seek_by_bytes) {
                    pos = -1;
...
void show_help_default(const char *opt, const char *arg)
{
    ...
    show_usage();
    show_help_options(options, "Mains options:", 0, OPT_EXPERT, 0);
...
//以音频时钟为基准：视频慢了则丢掉部分视频帧（视觉->画⾯跳帧） 视频快了则继续渲染上⼀帧
int main(int argc, char **argv)
{
    ...
    flush_pkt.data = (uint8_t *)&flush_pkt; // 初始化为数据指向自己本身,静态变量未初始化就是0.
